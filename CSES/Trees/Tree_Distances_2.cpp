...
int subtreeSum[200001];

// this will calculate ans for 1 and subtreeSum for rest of the nodes
...
void dfs2(int node, int parent, int partial_ans){
    ans[node] = subtreeSum[node] + (partial_ans + n - subtreeSize[node]);
    // cout << ans[node] << " ";
    for(auto &child : adj[node]){
        if(child == parent) continue;
        dfs2(child, node, ans[node] - subtreeSize[child] - subtreeSum[child]);
    }
}
void Shiremonoga(){
    cin >> n;
