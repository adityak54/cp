...
void dfs(int node, int parent, int currDist, vector<int> &ans, vector<int> adj[]){
    ans[node] = max(ans[node], currDist);
    for(auto &child : adj[node]){
        if(child == parent) continue;
        dfs(child, node, currDist + 1, ans, adj);
    }
...
void Shiremonoga(){
    ...
    int leaf1 = bfs(0, adj);
    int leaf2 = bfs(leaf1, adj);
    vector<int> ans(n);
    dfs(leaf1, -1, 0, ans, adj);
    dfs(leaf2, -1, 0, ans, adj);
    print(ans);
 }
signed main()
{
