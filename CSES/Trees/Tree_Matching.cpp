...
//-------------------------CODE----------------------
void dfs(int node, int parent, vector<vector<int>> &dp, vector<int> adj[]){
    ...
    if(!suffix.empty()) dp[node][0] = suffix[0];
     // calculating if we take node as one end
     int childCount = 0;
     for(auto &child : adj[node]){
        if(child == parent) continue;
        int leftSum = (childCount > 0) ? prefix[childCount - 1] : 0;
        int rightSum = (childCount < suffix.size()-1) ? suffix[childCount + 1] : 0;
        dp[node][1] = max(dp[node][1], 1 + leftSum + rightSum + dp[child][0]);
        childCount++;
     }
}
void Shiremonoga(){
