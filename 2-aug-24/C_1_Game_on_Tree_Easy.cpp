...
int n, t, stone;
int dist1 = -1, dist2 = -1;
void f(int node, int currDist,int parent,vector<int> adj[]){
    if(adj[node].size() == 1){
        if(dist1 == -1) dist1 = currDist;
        else dist2 = currDist;
    }
    for(auto &nextNode : adj[node]){
        if(nextNode != parent) f(nextNode, currDist+1, node, adj);
    }
...
void Shiremonoga(){
    ...
    cin >> stone;
    f(stone,0,-1,adj);
    if((dist1&1) || (dist2&1)) cout << "Ron" << endl;
