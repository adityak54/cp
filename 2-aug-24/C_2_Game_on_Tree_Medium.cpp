...
//-------------------------CODE----------------------
int n, t, stone;
void dfs(int node, int parent, vector<bool> &canRon,vector<int> adj[]){
    for(auto &nextNode : adj[node]){
        if(nextNode != parent){
            dfs(nextNode, node, canRon,adj);
            if(canRon[nextNode] == 0) canRon[node] = 1;
        }
    }
}
void Shiremonoga(){
    ...
    cin >> stone;
    // assume that at leaf node, it is Hermione's chance.
    // backtrack along each leaf and assign the chance resp.
    // if at node which holds stone, Ron's chance occur in any of the path, Ron can win.
    vector<bool> canRon(n+1,false);
    dfs(stone, 0, canRon,adj);

    if(canRon[stone]) cout << "Ron" << endl;
