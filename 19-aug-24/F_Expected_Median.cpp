...
int nCrMod(int n, int r) {
    ...
    int denominator = (mod_mul(fact[r], fact[n - r], mod)) % mod;
    return mod_mul(numerator, mod_inv(denominator, mod), mod);
}
void Shiremonoga(){
    int n, k;
    cin >> n >> k;
    ...
    int min1 = k/2+1, max0 = k/2;
    while(min1 <= cnt1 && max0 >= 0){
        ans = (ans + mod_mul(nCrMod(cnt1,min1),nCrMod(cnt0,max0),mod)) % mod;
        min1++, max0--;
    }
    cout << ans << endl;
 }
