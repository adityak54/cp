...
//-------------------------CODE----------------------
int n;
int findEnd(int root, vector<int> adj[]){
    int ans = -1;
    queue<pi> q;
    q.push({root,-1});
    while(!q.empty()){
...
void findDist(int root, vector<int> &dist, vector<int> adj[]){
    queue<pi> q;
    int cnt = 0;
    q.push({root, -1});
    while(!q.empty()){
...
void Shiremonoga(){
    ...
    int end1 = findEnd(1, adj);
    int end2 = findEnd(end1, adj);
    // cout << end1 << " " << end2 << endl;
    vector<int> dist(n+1);
    findDist(end1, dist, adj);
    findDist(end2, dist, adj);
    sort(all(dist));
    // print(dist);
    for(int k = 1; k <= n; k++){
        int ind = lower_bound(dist.begin(), dist.end(), k) - dist.begin();
        if(ind >= n) cout << n << " ";
        else cout << ind  << " ";
    }
 }
