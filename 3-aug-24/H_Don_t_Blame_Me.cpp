...
int n, k;
int mod = 1e9+7;
int f(int i, int currAnd, vector<int> &a, vector<vector<int>> &dp){
    if(i == n) return __builtin_popcount(currAnd) == k;
    if(dp[i][currAnd] != -1) return dp[i][currAnd];
    int take = f(i+1, currAnd & a[i], a, dp) % mod;
    int notTake = f(i+1, currAnd, a, dp) % mod;
    return dp[i][currAnd] = (take + notTake) % mod;
}
