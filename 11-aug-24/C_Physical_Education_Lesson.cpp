...
void Shiremonoga(){
    ...
    //  n + x - 2 = (k-1)*(2t + 2)
    //  n + x -2 = (2k-2)*m
    // 2k -2  = factors of n+x-2
    vector<int> div2 = divisor(n+x-2);
    unordered_set<int> s;
    fr(i,0,div1.size(),1){
        if(div1[i] % 2 == 0 && div1[i]/2 + 1 >= x) s.insert(div1[i] / 2 + 1);
    }
    fr(i,0,div2.size(),1){
        if(div2[i] % 2 == 0 && div2[i]/2 + 1 >= x) s.insert(div2[i] / 2 + 1);
    }
    cout << s.size() << endl;
 }
signed main()
