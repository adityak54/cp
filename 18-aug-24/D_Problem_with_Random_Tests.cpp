...
/* Why brute force works?
   -> because the string generation is random
   -> brute force will give O(n^2) if first 0 occurs at n/2th position
      as then we will have to check n/2 strings, but occurence of first 0
      at n/2th position has a probability of (1/(2^n)), which is almost 0 for 
      larger values of n.
*/
void Shiremonoga(){
    int n;
    ...
    string ans = s;
    int lengthOfS1 = n - start;
    int lengthOfS2 = lengthOfS1 - (ind- start);
    for(int i = start; i < ind; i++){
        string s1 = s.substr(start);
        string s2 = s.substr(i, lengthOfS2);
        for(int j = 0; j < lengthOfS2; j++){
            s1[j+ind-start] = max(s1[j+ind-start],s2[j]);
        }
        ans = max(ans, s1);
    }
    cout << ans << endl;
