...
void Shiremonoga(){
    ...
    for(auto &it : m){
        ...
        for(int i = 1; i < it.second.size(); i++){
            temp[i] = it.second[i] + temp[i-1];
        }
        for(int i = 0; i < temp.size(); i++){
            int s = ((((it.second.size() - i - 1)*n)%mod) - (temp.back()-temp[i]));
            ans = (ans + ((it.second[i] + 2) * s) % mod) % mod;
        }
        if(it.first == 0) ans = (ans + ((it.second.size()*n) % mod) - temp.back()) % mod;
    }
    cout << ans << endl;
 }
signed main()
