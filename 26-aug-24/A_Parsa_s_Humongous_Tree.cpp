...
//-------------------------CODE----------------------
void f(int node,int parent, vector<int> &l, vector<int> &r,
vector<int> adj[], vector<vector<int>> &dp){
    for(auto &child : adj[node]){
        if(child == parent) continue;
        f(child, node, l, r, adj, dp);
        dp[node][0] += max(dp[child][0] + abs(l[node] - l[child]),
                      dp[child][1] + abs(l[node] - r[child]));
        dp[node][1] += max(dp[child][0] + abs(r[node] - l[child]),
                      dp[child][1] + abs(r[node] - r[child]));
    }
