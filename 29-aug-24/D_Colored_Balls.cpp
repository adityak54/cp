...
int mod = 998244353;
int n;
int f(int i, int balls, vector<int> &a, vector<vector<int>> &dp){
    if(i==n) return 0;
    if(dp[i][balls] != -1) return dp[i][balls];
    int notTake = f(i+1, balls, a, dp);
    int ans = max((a[i] + balls + 1) / 2, a[i]);
    int take = (ans + f(i+1, balls + a[i], a, dp)) % mod;
    return dp[i][balls] = (take + notTake) % mod;
}
void Shiremonoga(){
    cin >> n;
