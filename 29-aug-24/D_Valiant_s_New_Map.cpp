...
void Shiremonoga(){
    ...
    // calculating log vector and intializing sparse table
    int LOG = log2(min(n,m)) + 1;
    vector<int> bin_log(min(n,m) + 1);
    bin_log[1] = 0;
    for(int i = 2; i <= min(n,m); i++){
        bin_log[i] = 1 + bin_log[i / 2];
    }
    
    vector<vector<vector<int>>> sparse(n, vector<vector<int>>(m, vector<int>(LOG))); // m[i][log] -> m[i][j][log]
    for(int i = 0; i < n; i++){
        for(int j = 0; j < m; j++){
            sparse[i][j][0] = a[i][j];
        }
    ...
    for(int k = 1; k <= LOG; k++){
        for(int i = 0; i + (1<<k) - 1 < n; i++){
            for(int j = 0; j + (1<<k) -1 < m; j++){
               // Combining four submatrices of size 2^(k-1) x 2^(k-1)
                sparse[i][j][k] = min({
                    sparse[i][j][k-1], // Top-left submatrix
                    sparse[i + (1 << (k-1))][j][k-1], // Bottom-left submatrix
                    sparse[i][j + (1 << (k-1))][k-1], // Top-right submatrix
                    sparse[i + (1 << (k-1))][j + (1 << (k-1))][k-1] // Bottom-right submatrix
                }); 
            }
        }
    }
    ...
    while(start <= end){
        ...
        for(int i = 0; i + mid <= n; i++){
            for(int j = 0; j + mid <= m; j++){
                if(min({
                    sparse[i][j][k],
                    sparse[i+mid-(1<<k)][j][k],
                    sparse[i][j+mid-(1<<k)][k],
                    sparse[i+mid-(1<<k)][j+mid-(1<<k)][k]})
                    >=mid){
                        chk = true;
                        break;
                }
