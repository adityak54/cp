...
void Shiremonoga(){
    ...
    int ans = 0;
    // finding the contribution of each bit in answer
    for(int i = 0; i < 31; i++){
        int indicesSum[2] = {-1, 0}; // indicesSum[0] = sum of indices whose ith bit is 0
                                     // indicesSum[1] = sum of indices whose ith bit is 1   
        int cnt[2] = {1, 0};         // cnt[0] = cnt of indices whose ith bit is 0
                                     // cnt[1] = cnt of indices whose ith bit is 1
        vector<int> b;
        for(int j = 0; j < n; j++){
            if((a[j]>>i)&1) b.push_back(1);
            else b.push_back(0);
        }
        int prev = 0;
        int currAns = 0;
        for(int j = 0; j < n; j++){
            b[j] = (b[j] + prev) % 2;
            currAns = (currAns + cnt[1-b[j]]*j  - indicesSum[1-b[j]]) % mod;
            indicesSum[b[j]] += j;
            cnt[b[j]] += 1;
            prev = b[j];
        }
        ans = (ans + (currAns*(1<<i)) % mod) % mod;
    }
    cout << ans << endl;
 }
signed main()
