...
void Shiremonoga(){
    ...
    vector<pi> v;
    for(auto &it : m) if(it.first < n) v.push_back(it);
    int currMex = 0;
    for(auto it : m){
        if(it.first != currMex) break;
        currMex++;     
    }
    // printContainer(v);
    // dp[i] represents minimum m if mex = i;
    vector<int> dp(currMex+1, INT_MAX);
    dp[currMex] = 0;
    for(int i = currMex; i >= 1; i--){
        for(int j = 0; j < i; j++){
            dp[j] = min(dp[j], dp[i] + (i * v[j].second));
        }
    }
    // print(dp);
    cout << dp[0] - currMex << endl;
 }
signed main()
{
