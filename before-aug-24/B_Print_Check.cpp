    ...
    void Shiremonoga(){
        ...
        int a[k][3];
        fr(i,0,k,1){
            fr(j,0,3,1) cin >> a[i][j];
        }
        vector<vector<int>> vis(3,vector<int>(max(n,m)+1)),ans(n,vector<int>(m));
        rfr(i,k-1,0,1){
            int type=a[i][0],RorC=a[i][1]-1,color=a[i][2];
            if(vis[type][RorC]) continue;
            else vis[type][RorC] = 1;
            if(type==1){
                fr(j,0,m,1) if(!ans[RorC][j]) ans[RorC][j] = color;
            }else{
                fr(j,0,n,1) if(!ans[j][RorC]) ans[j][RorC] = color;
            }
        }
        fr(i,0,n,1){
            fr(j,0,m,1) cout<<ans[i][j]<<" ";
            cout<<endl;
        }
    }
    signed main()
