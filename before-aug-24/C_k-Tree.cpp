...
//-------------------------CODE----------------------
int MOD = 1e9+7;
int f(int n,int k,int d,int check,vector<vector<int>> &dp){
    if(n<0) return 0;
    if(n==0 && check) return 1;
    if(n==0 && !check) return 0;
    if(dp[n][check]!=-1) return dp[n][check];
    int ans = 0;
    int original = check;
    for(int i=1;i<=min(n,k);i++){
        if(i>=d) check=1;
        ans=(ans+(f(n-i,k,d,check,dp)%MOD))%MOD;
    }
    return dp[n][original]=ans;
}
void Shiremonoga(){
    int n,k,d;
    cin >> n >> k >> d;
