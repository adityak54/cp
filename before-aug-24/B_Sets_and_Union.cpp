...
//-------------------------CODE----------------------
// recursion (TLE dega)
// int f(int i,map<int,int> m,map<int,int> &u,vector<vector<int>> &a){
//     int n = a.size();
//     if(i==n){
//         for(auto &it : u){
//             if(m.find(it.first)==m.end()) return m.size();
//         }
//         return INT_MIN;
//     }
//     int notTake = f(i+1,m,u,a);
//     fr(j,0,a[i].size(),1) m[a[i][j]]++;
//     int take = f(i+1,m,u,a);
...
void Shiremonoga(){
    ...
    for(auto &it : s){
        ...
        fr(i,0,n,1){
            ...
            if(!check){
                fr(j,0,a[i].size(),1) st.insert(a[i][j]);
            }
        }
