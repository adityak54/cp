...
void Yowaimo(){
    ...
    for(int i=0; i<q; i++){
        ...
        int d = (x*x)-4*y;
        int rootd = sqrt(d);
        if(rootd*rootd!=d){ans.push_back(0); continue;}
        int root1 = (x-rootd)/2;
        int root2 = (x+rootd)/2;

        if(root1==root2){
            if(m.find(root1)!=m.end()){
                ans.push_back((m[root1]*(m[root1]-1))/2);
            }
            else ans.push_back(0);
        }
        else{
            if(m.find(root1)!=m.end() && m.find(root2)!=m.end()){
                ans.push_back(m[root1]*m[root2]);
            }
            else{
                ans.push_back(0);
            }
