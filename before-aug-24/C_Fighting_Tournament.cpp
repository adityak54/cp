...
void Shiremonoga(){
    ...
    fr(0,q-1,1){
        ...
        k-=max((int)0,ind-1);
        k=max((int)0,k);
        if(a[ind]==n){cout<<k<<endl; continue;}
        if(!canWin[ind]) cout << 0 << endl;
        else{
           if(ind==0) cout << min(k,max((int)0,nextG[ind]-ind-1))<<endl;
           else cout << min(k,max((int)0,nextG[ind]-ind)) << endl;
        }
    }
 }
signed main()
