...
void Shiremonoga(){
    ...
    cin >> n >> m >> k;
    if(k>n || k==1){
        // k>n hai to 0 substrings possible hain
        // wo saare palindrome honge
        // Therefore saare strings of size n ans honge
        int ans = 1;
        fr(i,0,n,1){
            ans = (ans*m)%mod;
        }
        ...
        // substring ->            _ _ _ _ _ _ _
        // ways of selecting ->    m m m m 1 1 1 -> m^(ceil(n/2))
        int ans = 1;
        fr(i,0,(n+1)/2,1){
            ans = (ans*m)%mod;
        }
