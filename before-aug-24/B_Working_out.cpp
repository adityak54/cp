...
void Shiremonoga(){
    ...
    int a[n+1][m+1];
    fr(i,1,n+1,1){
        fr(j,1,m+1,1) cin >> a[i][j];
    }
    int dp1[n+2][m+2], dp2[n+2][m+2], dp3[n+2][m+2], dp4[n+2][m+2];
    memset(dp1,0,sizeof(dp1));
    memset(dp2,0,sizeof(dp2));
    memset(dp3,0,sizeof(dp3));
    memset(dp4,0,sizeof(dp4));
    // max cost from (0,0) to meeting point Iahub (can move right and down):
    fr(i,1,n+1,1){
        fr(j,1,m+1,1){
            int up = dp1[i-1][j];
            int left = dp1[i][j-1];
            dp1[i][j] = a[i][j]+max(left,up);
        }
    }

    // max cost from meeting point to (n-1,m-1) for Iahub (can move right and down):
    rfr(i,n,1,1){
        rfr(j,m,1,1){
            int right = dp2[i][j+1];
            int down = dp2[i+1][j];
            dp2[i][j] = a[i][j]+max(right,down);
        }
    }
    ...
    fr(i,1,n+1,1){
        rfr(j,m,1,1){
            int down = dp4[i-1][j];
            int left = dp4[i][j+1];
            dp4[i][j] = a[i][j]+max(down,left);
        }
    }
    int ans = 0;
    fr(i,2,n,1){
        fr(j,2,m,1){
            // i,j is the meeting point
