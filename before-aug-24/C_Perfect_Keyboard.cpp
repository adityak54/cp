...
//-------------------------CODE----------------------
bool isCycle(int node,int parent,vector<int> &visited,set<int> adj[]){
    visited[node] = 1;
    for(auto it: adj[node]){
        if(!visited[it]){
            visited[it]=1;
            if(isCycle(it,node,visited,adj)) return true;
        }
        else if(it!=parent && visited[it]) return true;
    }
    return false;
}
// void dfs(int node,vector<int> &visited, set<int> adj[]){
//     visited[node] = 1;
//     for(auto it: adj[node]){
//         if(!visited[it]){
//             dfs(it,visited,adj);
...
void Shiremonoga(){
    ...
    fr(i,0,26,1){
        if(adj[i].size()>2){cout<<"NO"<<endl; return;}
    }
    // fr(i,0,26,1){
    //     cout << i << "-> ";
    ...
    if(isCycle(s[0]-'a',-1,visited,adj)){cout<<"NO"<<endl; return;}
    cout << "YES" << endl;
    fr(i,0,26,1) visited[i]=0;

    // dfs(s[0]-'a',visited,adj);
    int src = s[0]-'a';
    fr(i,0,26,1){
        if(adj[i].size()==1){
            src = i;
