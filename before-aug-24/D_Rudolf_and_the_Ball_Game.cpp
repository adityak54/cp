...
//-------------------------CODE----------------------
int visited[1001];
void f(int i,int x,int m,int n, vector<pair<int,string>> &a,vector<vector<int>> &dp){
    if(i==m){
        visited[x] = 1;
        return;
    }
...
void Shiremonoga(){
    ...
    fr(i,1,n+1,1) visited[i] = 0;
    vector<vector<int>> dp(n+1,vector<int>(m+1,-1));
    f(0,x,m,n,a,dp);
    int ans = 0;
    fr(i,1,n+1,1) ans+=visited[i];
    cout << ans << endl;
    fr(i,1,n+1,1) if(visited[i]) cout << i << " ";
    cout << endl;
 }
signed main()
{
