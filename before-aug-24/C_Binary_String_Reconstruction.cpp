...
void Shiremonoga(){
    ...
    for(int i=0;i<n;i++){
        if(s[i]=='0' && i-x>=0) ans[i-x] = '0';
        if(s[i]=='0' && i+x<n) ans[i+x] = '0';
    }
    ...
    for(int i=0;i<n;i++){
        ...
        if(i-x>=0) checkl = (ans[i-x]=='1');
        if(i+x<n) checkr = (ans[i+x]=='1');
        if(s[i]=='0' && !checkl && !checkr) continue;
        if(s[i]=='1' && (checkl || checkr)) continue;
        cout<<-1<<endl; return;
    }
