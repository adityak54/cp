...
void Shiremonoga(){
    ...
    fr(i,0,2*n,1){
        ...
        m[card[1]].pb(card[0]-'0');
    }
    int cntodd = 0;
    
    for(auto &it:m){
        sort(all(it.second));
        if(it.second.size()&1 && it.first!=trump) cntodd++;
    }
    if(cntodd>m[trump].size() || ((m[trump].size()-cntodd)&1)){cout<<"IMPOSSIBLE"<<endl; return;}
    int ind = 0;
    for(auto &it:m){
        if(it.first==trump) continue;
        if(it.second.size()&1){
            cout<<it.second.back()<<it.first << " ";
            cout<<m[trump][ind]<<trump<<endl;
            ind++;
        }

        fr(i,0,it.second.size()-1,2){
            cout<<it.second[i]<<it.first<<" ";
            cout<<it.second[i+1]<<it.first<<endl;
        }
    }
    if((m[trump].size()-ind)%2!=0){cout<<"IMPOSSIBLE"<<endl; return;}
    sort(all(m[trump]));
    fr(i,ind,m[trump].size(),2){
        cout<<m[trump][i]<<trump<<" ";
        cout<<m[trump][i+1]<<trump<<endl;
    }
