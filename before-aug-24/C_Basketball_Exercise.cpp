...
int a[100100][2],n;
//-------------------------CODE----------------------
int f(int i,int prev,vector<vector<int>> &dp){
    if(i==n) return 0;
    if(dp[i][prev]!=-1) return dp[i][prev];

    int take=0, notTake = 0;
    if(prev==2){
        take = max(a[i][1]+f(i+1,1,dp),a[i][0]+f(i+1,0,dp));
        notTake = f(i+1,2,dp);
    }else{
        take = a[i][!prev]+f(i+1,!prev,dp);
        notTake = f(i+1,2,dp);
    }
...
void Shiremonoga(){
    cin >> n;
    fr(0,n-1,1) cin >> a[i][0];
    fr(0,n-1,1) cin >> a[i][1];
