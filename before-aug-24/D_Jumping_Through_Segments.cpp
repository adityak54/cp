...
//-------------------------CODE----------------------
bool check(int mid, int n, vector<pi> &a){
    int low = 0, high = mid;
    fr(i,0,n,1){
        if(low>a[i].second || high<a[i].first) return false;
        int minReachable = max(a[i].first,low);
        int maxReachable = min(a[i].second,high);
        low = max((int)0,minReachable-mid);
        high = maxReachable+mid;
    }
    return true;
}
