    ...
    class SegmentTree{
        ...
        void build(int ind, int low, int high, vector<int> &arr,int m){
            if(high==low){
                seg[ind]=arr[low]%m;
                return;
            }
            int mid = (low+high)>>1;
            build(2*ind+1,low,mid,arr,m);
            build(2*ind+2,mid+1,high,arr,m);
            seg[ind] = (seg[2*ind+1]*seg[2*ind+2])%m;
        }
        int query(int ind,int low, int high, int l, int r, int m){
            // no overlap
            ...
            // partial overlap
            int mid = (low+high)>>1;
            int left = query(2*ind+1,low,mid,l,r,m)%m;
            int right = query(2*ind+2,mid+1,high,l,r,m)%m;
            return (left*right)%m;
        }
    };
    void Shiremonoga(){
        ...
        string s; cin >> s;
        fr(i,0,n,1) a[i] = a[i]%m;
        SegmentTree SG = SegmentTree(n);
        SG.build(0,0,n-1,a,m);
        int start = 0, end = n-1;
        fr(i,0,n,1){
            cout << SG.query(0,0,n-1,start,end,m) << " ";
            if(s[i]=='L') start++;
            else end--;
        }
        cout << endl;
