...
void Shiremonoga(){
    ...
    cin >> n;
    int mx=0,mn=1e9;
    int costmx = 0, costmn = 0;
    int mxLen = 0, costLen = INT_MAX;
    fr(i,0,n,1){
        int l,r,cost;
        cin >> l >> r >> cost;
        if(l<=mn){
            if(l==mn){
                if(cost<costmn) costmn = cost;
            }else{
                costmn = cost;
                mn = l;
            }
        ...
        if(currLen>mxLen){
            mxLen = currLen;
            costLen = cost;
        }else if(currLen==mxLen) costLen = min(cost,costLen);

        int currCost = costmn+costmx;
        if(mxLen==mx-mn+1) currCost = min(currCost,costLen);
        cout << currCost << endl;
    }
 }
signed main()
