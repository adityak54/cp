...
int checka[2001];
int checkb[2001];
int f(bool check,int x, vector<pi> &a, vector<pi> &b){
    memset(checka,1,sizeof(checka));
    memset(checkb,1,sizeof(checkb));
...
void Shiremonoga(){
    ...
    int ans = f(false,x,a,b);
    ans = max(ans,f(true,x,a,b));
    // printContainer(a);
    // printContainer(b);
    cout << ans << endl;
 }
