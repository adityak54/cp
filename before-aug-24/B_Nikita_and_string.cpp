...
//-------------------------CODE----------------------
string s;
int f(int i,char prev,int n,int cnt,vector<vector<int>> &dp){
    if(i==n) return 0;
    if(dp[i][cnt]!=-1) return dp[i][cnt];
    int take = 0;
    int tempCnt = cnt;
    if(s[i]!=prev){
        if(s[i]=='b') cnt = max(cnt+1,(int)2);
        else cnt++;
        if(cnt<4) take=1+f(i+1,s[i],n,cnt,dp);
    }
    else take=1+f(i+1,s[i],n,cnt,dp);
    int notTake = f(i+1,prev,n,tempCnt,dp);
    return dp[i][tempCnt] = max(take,notTake);
}
void Shiremonoga(){
    cin >> s;
    int n = s.size();
