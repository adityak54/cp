...
int cost[3],curr[3]; 
string s;
bool check(int mid, int rokda,vector<int> ing){
    int ans = 0,mn = INT_MAX;
    fr(i,0,3,1) {
        if(curr[i]) mn = min(mn,ing[i]/curr[i]);
        else ing[i]=0;
    }
    ans+=mn;
    fr(i,0,3,1) ing[i]-=mn*curr[i];
    int mx = 0;
    fr(i,0,3,1) if(curr[i]) mx = max(mx,ing[i]/curr[i]);
    
    while(ing[0] || ing[1] || ing[2]){
        fr(i,0,3,1){
            if(curr[i]>ing[i]){
                if(rokda<(curr[i]-ing[i])*cost[i]) return ans>=mid;
                else rokda-=(curr[i]-ing[i])*cost[i], ing[i]=curr[i];
            }
        }
        ans++;
        fr(i,0,3,1) ing[i]-=curr[i];
    }
    int moneyReq = 0;
    fr(i,0,3,1) moneyReq+=curr[i]*cost[i];
    ans+=rokda/moneyReq;
    return ans>=mid;
...
void Shiremonoga(){
    ...
    int n = s.size();
    vector<int> ing(3);
    fr(i,0,3,1) cin >> ing[i];
    fr(i,0,3,1) cin >> cost[i];
    int rokda; cin >> rokda;
    fr(i,0,n,1){
        if(s[i]=='B') curr[0]++;
        else if(s[i]=='S') curr[1]++;
        else curr[2]++;
    }
    int ans = 0;
