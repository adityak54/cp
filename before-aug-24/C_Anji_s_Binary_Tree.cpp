...
//-------------------------CODE----------------------
int dfs(int node,vector<pi> &a, string &s,vector<int> &visited){
    node--;
    int left = INT_MAX, right = INT_MAX;
    visited[node] = 1;
    if(a[node].first!=0 && !visited[a[node].first-1]){
        left = dfs(a[node].first,a,s,visited);
        if(s[node]!='L') left++;
    }
    if(a[node].second!=0 && !visited[a[node].second-1]){
        right = dfs(a[node].second,a,s,visited);
        if(s[node]!='R') right++;
    }
