    ...
    //-------------------------CODE----------------------
    void dfs(int node,int parent,vector<int> &dp,vector<int> &childCount,vector<int> adj[]){
        childCount[node] = 1;
        int left = -1, leftNode = 0;
        int right = -1, rightNode = 0;
        for(auto it:adj[node]){
            if(it!=parent){
                dfs(it,node,dp,childCount,adj);
                if(left==-1) left=dp[it], leftNode = it;
                else right = dp[it], rightNode = it;
                childCount[node]+=childCount[it];        
            }
        }
        dp[node] = max(max((int)0,left)+childCount[rightNode]-1,
                        max((int)0,right)+childCount[leftNode]-1);
    }
    void Shiremonoga(){
        int n; cin >> n;
        vector<int> adj[n+1];
        ...
        vector<int> dp(n+1), childCount(n+1);
        dfs(1,0,dp,childCount,adj);
        cout << dp[1] << endl;
