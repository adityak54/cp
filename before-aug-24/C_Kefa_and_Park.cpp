...
//-------------------------CODE----------------------
int n,m;
int f(int node,int currCats,vector<int> &visited,vector<int> &cats,vector<int> adj[]){
    visited[node] = 1;
    ...
    for(auto &nextNode: adj[node]){
        if(!visited[nextNode]){
            ans+=f(nextNode,currCats,visited,cats,adj);
        }
    }
    return ans;
}
void Shiremonoga(){
    ...
    int ans = f(1,0,visited,cats,adj);
    cout << ans << endl;
 }
signed main()
{
