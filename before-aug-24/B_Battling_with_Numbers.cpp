    ...
    //-------------  CODE STARTS  -------------------
    int mod = 998244353;
    int power(int number, unsigned int kitnaPowerMe, int mod) 
    { 
        ...
        if (number == 0) return 0; // In case x is divisible by p;
    
        while (kitnaPowerMe > 0) 
        { 
            ...
            // y must be even now 
            kitnaPowerMe = kitnaPowerMe>>1; // y = y/2 
    ...
    void Yowaimo(){
        ...
        map<int,int> a;
        map<int,int> b;
        vector<int> x=input<int>(n);
        vector<int> px=input<int>(n);
        for(int i=0;i<n;i++){
            a.insert({x[i],px[i]});
        }
        int m; cin>>m;
        vector<int> y=input<int>(m);
        vector<int> py=input<int>(m);
        for(int i=0;i<m;i++){
            b.insert({y[i],py[i]});
        }
        int ans = 0;
        for(auto it : b){
            // y-> hcf hai therefore usme aisa koi factor nahi aana chahiye jo x me na ho
            if(a.find(it.first)==a.end() || a[it.first]<it.second){cout<<0<<endl; return;}
            a[it.first]-=it.second;
        }
        for(auto it : a) if(it.second>0) ans++;
        cout<<(power(2,ans,mod))%mod<<endl;
    }
    signed main()
    {
