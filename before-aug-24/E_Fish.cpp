...
double fishes[19][19];
double dp[1<<19];
double getProb(int fish, int mask){
    //  jitne alive fish hain unme aise pairs jinme 'fish' to ho hi
    //  aur wo marr jaaye
    int k = __builtin_popcount(mask);
    double deno = (k*(k-1))/2;  // k alive me se total pairs
    double nume = 0;
    for(int i=0; i<n; i++){
        if((mask&(1<<i))) nume += fishes[i][fish];
    }
...
double f(int mask){
    ...
    fr(fish,0,n,1){
        // 0 -> dead
        bool dead = !(mask&(1<<fish));
        if(dead){
            // ith fish ko maarne ki probability
            int prevMask = mask^(1<<fish);
            double moveProb = getProb(fish, prevMask);
            double prevDayProb = f(prevMask);
            ans += moveProb*prevDayProb;
        }
    }
    return dp[mask]=ans;
}
void Shiremonoga(){
    cin >> n;
    fr(i,0,n,1){
        fr(j,0,n,1) cin >> fishes[i][j];
    }
    memset(dp,-1,sizeof dp);
    fr(i,0,n,1) cout<<fixed<<setprecision(6)<<f((1<<i))<<" ";
 }
signed main()
{
