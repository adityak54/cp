...
void Shiremonoga(){
    ...
    // saare digits ka sum karna hai
    // ye operation jyada baar nahi lagega isliye brute force hi mar denge
    // kispe kispe lag sakta hai wo indices ko track karna hai 
    int n,q;
    cin >> n >> q;
    vector<int> a=input<int>(n);
    set<int> s;
    fr(i,0,n,1) if(a[i]>9) s.insert(i);
    fr(i,0,q,1){
        int t;
        cin >> t;
        if(t==1){
