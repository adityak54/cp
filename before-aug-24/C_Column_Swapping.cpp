...
void Shiremonoga(){
    ...
    fr(i,0,n,1){
        fr(j,0,m,1) cin >> a[i][j];
    }
    vector<vector<int>> invalid(n);
    pi req={-1,-1};
    fr(i,0,n,1){
        int f=-1, s=-1;
        vector<int> temp = a[i];
        ...
        if(cnt==2) req.first = f, req.second = s;
    }

    if(req.first==-1 && req.second==-1){cout<<1<<" "<<1<<endl; return;}
    fr(i,0,n,1){
        swap(a[i][req.first],a[i][req.second]);
    }
    
    fr(i,0,n,1){
        fr(j,1,m,1) if(a[i][j]<a[i][j-1]) {cout<<-1<<endl; return;}
    }
    cout << req.first+1 << " " << req.second+1 << endl;
 }
signed main()
{
