...
void Shiremonoga(){
    cin >> n >> d;
    vector<pi> money(n),fship;
    fr(i,0,n,1){
        cin >> money[i].first >> money[i].second;
        fship.pb({money[i].second,money[i].first});
    }   
    sort(all(money));
    sort(all(fship),cmp);
    int ans = 0;
    ...
    while(j<n){
        curr+=money[j].second;
        while(i<j && money[j].first-money[i].first>=d){
            curr-=money[i].second;
            i++;
        }
        ans = max(ans,curr);
    ...
    cout << ans << endl;
    // below case is being covered already
    // int mx = 0;
    // int mn = 1e10;
    // int ans2 = 0;
    // fr(i,1,n,1){
    //     mx = max(mx,fship[i].second);
    //     mn = min(mn,fship[i].second);
    //     if(mx-mn>=d) break;
    //     ans2+=fship[i].first;
    // }
    // cout << max(ans,ans2) << endl;
    // printContainer(money);
    // printContainer(fship);
 }
signed main()
{
