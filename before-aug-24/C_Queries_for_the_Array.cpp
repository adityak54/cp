...
void Shiremonoga(){
    string s; cin >> s;
    int n = s.size();
    int curr = 0,maxSorted = 0, minNotSorted = INT_MAX;
    fr(i,0,n,1){
        if(s[i]=='+'){
            curr++;
            ...
            curr--;
            // agar minNotSorted=3 hai aur curr length hi 2 ho gaya
            // then we dont know the correct value of minNotSorted
            if(curr<minNotSorted) minNotSorted = INT_MAX;
            maxSorted = min(maxSorted,curr);
        }else if(s[i]=='1'){    
            if(minNotSorted<=curr){cout<<"NO"<<endl; return;}
            maxSorted = max(maxSorted,curr);
        }else{
            if(maxSorted>=curr || curr<2){cout<<"NO"<<endl; return;}
