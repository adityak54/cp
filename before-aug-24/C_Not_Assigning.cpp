...
//-------------------------CODE----------------------
void dfs(int node,int val,vector<pi> adj[],vector<int> &vis,vector<int> &ans){
    vis[node] = 1;
...
void Shiremonoga(){
    ...
    fr(i,1,n+1,1){
        if(adj[i].size()>2){cout<<-1<<endl; return;}
    }
    int leaf = 1;
    fr(i,0,n,1){
        if(adj[i].size()==1) {leaf = i; break;}
    }
    vector<int> ans(n-1);
    vector<int> vis(n+1);
    dfs(leaf,2,adj,vis,ans);
    print(ans);
 }
signed main()
{
