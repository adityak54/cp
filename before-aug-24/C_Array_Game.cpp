    ...
    if(k==2){
        fr(i,0,n,1){
            fr(j,i+1,n,1){
                int curr = a[j]-a[i];
                mn = min(curr,mn);
                int ind = lower_bound(all(a),curr)-a.begin();
                mn = min(mn,abs(curr-a[ind]));
                if(ind>0) mn = min(mn,abs(curr-a[ind-1]));
                if(ind<n-1) mn = min(mn,abs(curr-a[ind+1]));
            }
        }
    }
    cout << mn << endl;
