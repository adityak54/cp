...
void Shiremonoga(){
    ...
    sortall(a);
    int case1 = a[n-1]*a[n-2]*a[n-3]*a[n-4]*a[n-5]; // all neg
    int case2 = a[n-1]*a[0]*a[1]*a[2]*a[3]; // 1 pos 4 neg
    int case3 = a[n-1]*a[n-2]*a[0]*a[1]*a[n-3]; // 3 pos 2 neg

    // baaki ke cases
    // all pos -> case1,
    // only 5 elements waala sab cover ho gaya
