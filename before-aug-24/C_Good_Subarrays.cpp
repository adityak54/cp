...
void Shiremonoga(){
    ...
    string s; cin >> s;
    int ans = 0;
    vector<int> prefSum(n), rprefSum;
    prefSum[0] = s[0]-'0';
    fr(i,1,n,1) prefSum[i] = prefSum[i-1]+(s[i]-'0');
    rprefSum.pb(s[n-1]-'0');
    fr(i,1,n,1) rprefSum.pb(rprefSum.back()+(s[n-i-1]-'0'));
    fr(i,0,n-1,1){
        if(prefSum[i]==i+1) ans++;
        if(rprefSum[i]==i+1) ans++;
    }
    if(prefSum[n-1]==n) ans++;
