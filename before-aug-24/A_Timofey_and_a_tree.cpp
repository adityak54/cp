    ...
    bool dfs(int node,int parent,vector<int> &color,vector<int> adj[]){
        ...
        for(auto it:adj[node]){
            ...
            else if(!dfs(it,node,color,adj)) return false;
        }
        return true;
    }
    bool checker(int root,vector<int> adj[],vector<int> &color){
        for(auto it:adj[root]){
            if(!dfs(it,root,color,adj)) return false;
        }
        return true;
    }
    void Shiremonoga(){
        ...
        vector<int> color=input<int>(n);
        
        fr(i,0,n,1){
            for(auto it:adj[i]){
                if(color[i]!=color[it]){
                    bool ans1 = checker(i,adj,color);
                    bool ans2 = checker(it,adj,color);
                    if(ans1) cout<<"YES"<<endl<<i+1;
                    else if(ans2) cout<<"YES"<<endl<<it+1;
                    else cout<<"NO"<<endl;
                    return;
                }
            }
