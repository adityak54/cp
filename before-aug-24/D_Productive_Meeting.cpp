...
void Shiremonoga(){
    ...
    int ans = 0;
    while(1){
        int first_val = p.top().first;
        int first_ind = p.top().second; p.pop();

        int second_val = p.top().first;
        int second_ind = p.top().second; p.pop();


        if(first_val<=0 || second_val<=0) break;

        ans+=1;
        p.push({first_val-1,first_ind});
        p.push({second_val-1,second_ind});
    }
    cout<<ans<<endl;
    while(1){
        int first_val = pq.top().first;
        int first_ind = pq.top().second; pq.pop();

        int second_val = pq.top().first;
        int second_ind = pq.top().second; pq.pop();

        if(first_val<=0 || second_val<=0) break;

        cout<<first_ind+1<<" "<<second_ind+1<<endl;
        
        pq.push({first_val-1,first_ind});
        pq.push({second_val-1,second_ind});
    }
 }
signed main()
{
