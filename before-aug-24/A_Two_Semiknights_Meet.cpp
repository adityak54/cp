...
int dp[8][8][8][8];
int visited[8][8][8][8];
bool f(int i1,int j1, int i2, int j2,vector<string> &board){
    if(i1==i2 && j1==j2 && board[i1][j1]!='#') return true;
    ...
    fr(i,0,4,1){
        fr(j,0,4,1){
            if(i==0 && j==0) continue;
            int ni1 = i1+deli1[i], nj1 = j1+delj1[i], ni2 = i2+deli2[j], nj2 = j2+delj2[j];
            if(ni1<0 || ni1>=8 || ni2<0 || ni2>=8 || nj1<0 || nj1>=8 || nj2<0 || nj2>=8) continue;
            if(visited[ni1][nj1][ni2][nj2]) continue;
            visited[ni1][nj1][ni2][nj2] = 1;
            bool ans = f(ni1,nj1,ni2,nj2,board);
            if(ans) return dp[i1][j1][i2][j2] = true;
        }
    }
