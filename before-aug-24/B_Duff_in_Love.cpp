...
template<typename T>void printContainer(const std::priority_queue<T>& pq) {std::priority_queue<T> div = pq;std::cout << "Priority Queue: ";while (!div.empty()) {std::cout << div.top() << " ";div.pop();}std::cout << std::endl;}
...
template<typename T> void printContainer(const std::queue<T>& q) {std::queue<T> div = q; std::cout << "Queue: ";while (!div.empty()) {std::cout << div.front() << " ";div.pop();}   std::cout << std::endl;}
template<typename A, typename B>void printContainer(const std::queue<std::pair<A, B>>& q) {std::queue<std::pair<A, B>> div = q;     std::cout << "Queue of Pairs: ";while (!div.empty()) {std::cout << "(" << div.front().first << ", " << div.front().second << ") ";div.pop();}std::cout << std::endl;}
template<typename A, typename B>void printContainer(const std::stack<std::pair<A, B>>& s) {std::stack<std::pair<A, B>> div = s; std::cout << "Stack of Pairs: ";while (!div.empty()) {std::cout << "(" << div.top().first << ", " << div.top().second << ") ";div.pop();}std::cout << std::endl;}
