...
void Shiremonoga(){
    ...
    int val2 = r*r;
    for(int x=0;x<=r;x++){
        double y = sqrt(val2 - x*x);  
        double y1 = sqrt(val - x*x);
        int integerPoints = ceil(y1)-y;
        if(x==0) ans+=2*integerPoints;
        else if(x==r) ans+=(4*(integerPoints-1))+2;
        else ans+=4*integerPoints;   
    }
    cout<<ans<<endl;
}
