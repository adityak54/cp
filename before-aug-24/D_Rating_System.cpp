...
void Shiremonoga(){
    ...
    // jaha pe bhi maximum negative ho raha (can be a subarray of negative integers)
    // waha tak ka sum hi ans hoga 

    int currSum = 0, maxSum = 0, maxDiff = 0, ans = 0;
    for(int i=0;i<n;i++){
        currSum+=a[i];
        maxSum = max(currSum,maxSum);
        int diff = maxSum-currSum;
        if(diff>maxDiff){
            maxDiff = diff;
            ans = maxSum;
        }
