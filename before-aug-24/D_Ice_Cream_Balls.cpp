...
void Yowaimo(){
    ...
    // for x, maximum possible cakes = x*(x-1)/2
    // say x*(x-1)/2 = n  -> x = sqrt(2*x) (approx)
    // ans will be greater than sqrt(2*x)

    for(int i = start; i<=2648956421; i++){
        int val = (i*(i-1))/2;
        if(val==n){cout<<i<<endl; return;}
        if(val>n){
            int temp = ((i-1)*(i-2))/2;
            cout<<n-temp+i-1<<endl;
            return;
        }
    }
}
