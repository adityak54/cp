...
bool cmp(pair<int, pi> x, pair<int, pi> y){if (x.first != y.first)return x.first > y.first; else return x.second.first < y.second.first; }
...
void Shiremonoga(){
    ...
    vector<int> b=input<int>(n);
    vector<pair<int,pi>> v(n);
    // 0 -> Bob , 1->Alice
    fr(i,0,n,1){
        if(a[i]>b[i]) v[i] = {a[i]-1,{1,i}};
        else if(a[i]==b[i]) v[i] = {a[i]-1,{-1,i}};
        else v[i] = {b[i]-1,{0,i}};
    }
    ...
    fr(i,0,n,1){
        ...
        else if(v[i].second.first==0 && i&1) ans-=v[i].first;
        else if(v[i].second.first==0 && !(i&1)) ans+=a[v[i].second.second]-1;
        else if(v[i].second.first==1 && !(i&1)) ans+=v[i].first;
        else if(v[i].second.first==1 && (i&1)) ans-=b[v[i].second.second]-1;
        // cout << ans << endl;
    }
    cout << ans << endl;
    // for(auto &it : v){
