...
//-------------------------CODE----------------------
int mod = 1e9+7;
void dfs(int node,vector<int> &vis,stack<int> &finishTime,vector<int> adj[]){
    vis[node] = 1;
    for(auto it:adj[node]){
        if(!vis[it]){
            dfs(it,vis,finishTime,adj);
        }
    }
    finishTime.push(node);
}
void dfs2(int node,vector<int> &vis,vector<int> &members,vector<int> revAdj[]){
    vis[node] = 1;
    for(auto &it:revAdj[node]){
        if(!vis[it]){
            dfs2(it,vis,members,revAdj);
        }
    }
    members.pb(node);
}
void Shiremonoga(){
    ...
    while(!finishTime.empty()){
        ...
        if(!vis[i]){
            vector<int> members;
            dfs2(i,vis,members,revAdj);
            map<int,int> m;
            fr(i,0,members.size(),1){
                m[cost[members[i]]]++;
            }
            ways = (ways*(((*m.begin()).second)%mod))%mod;
            totalCost +=((*m.begin()).first);
        }
    }
