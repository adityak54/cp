...
void Shiremonoga(){
    ...
    if(type=="+"){
        ...
        mleft.erase(mleft.find(l)); // deletes only 1 occurence of the l
        mright.erase(mright.find(r));
    }
    if(mleft.size()<=1){cout<<"NO"<<endl; return;}
    int leftMostEnd = *mright.begin();
    if(mleft.lower_bound(leftMostEnd+1)!=mleft.end()) cout<<"YES"<<endl;
    else cout<<"NO"<<endl;
 }
signed main()
{
