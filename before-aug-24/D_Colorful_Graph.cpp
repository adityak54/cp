...
void Shiremonoga(){
    ...
    int ans = -1;
    int cnt = -1;
    vector<set<int>> neighbourcnt(1e5+2);
    fr(i,0,n,1){
        for(auto it: adj[i]){
            if(colors[it]!=colors[i]){
                neighbourcnt[colors[i]].insert(colors[it]);
            }
        }
    }
    fr(i,0,n,1){
        int col = colors[i];
        // cout << neighbourcnt[col].size() << endl;
        if(neighbourcnt[col].size()>cnt){
            cnt = neighbourcnt[col].size();
            ans = col;
        }else if(neighbourcnt[col].size()==cnt){
            if(col<ans) ans = col;
        }
    }
    cout << ans << endl;
