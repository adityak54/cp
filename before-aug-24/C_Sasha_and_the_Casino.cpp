...
void Shiremonoga(){
    int k,x,a;
    cin >> k >> x >> a;
    if(x==1 && a==1){cout<<"NO"<<endl; return;}
    if(k==2 && x==1 && a==2){cout<<"NO"<<endl; return;}
    if(x<<(k+1)<a){cout<<"YES"<<endl; return;}
    if(x>k+1){cout<<"NO"<<endl; return;}
    int val = x-1;
    if(val>=a){cout<<"NO"<<endl; return;}
    int val2 = (a-val)/2;
    int val3 = a-val-val2;
    if(val2<0 || val3<0){cout<<"NO"<<endl; return;}
    if((a-val)%2==0) val2--;
    // cout << val << " " << val2 << " " << val3 << endl;

    if(((val2*k)>val2+val)){cout<<"YES"<<endl; }
    else cout<<"NO"<<endl;
    // int mx = a/x;   
    // fr(i,1,mx,1){
        // int val = x*i;
        // if(val>a) break;
        // int val2 = x*k;
        // if(val2>val && val<=a){cout<<"YES"<<endl; return;}
        // int b = 
    // }
    // cout<<"NO"<<endl;
