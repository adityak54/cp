...
void Shiremonoga(){
    ...
    if(n&1){
        int ans = 0;
        fr(1,n-2,2){
            ans+=max((int)0,max(a[i-1]-a[i],a[i+1]-a[i])+1);
        }
            cout << ans << endl; 
            return;
    }
    vector<int> prefCost(n);
    int ans = 0;
    fr(1,n-2,2){
        prefCost[i]=max((int)0,max(a[i-1]-a[i],a[i+1]-a[i])+1);
        ans+=prefCost[i];
    }
    int currCost = ans;
    rfr(n-2,1,2){
        int cost = max((int)0,max(a[i-1]-a[i],a[i+1]-a[i])+1);
        currCost-=prefCost[i-1];
        currCost+=cost;
    ...
    /*  _ T _ T _ T _ _  -> ye prefix me store hai

        ab ham piichhe se take wala shift karna shuru karenge
        NOTE -> jab ek baar 'T' shift hua to wahi pe hai aage ke cases me bhi
        _ T _ T _ _ T _
        _ T _ _ T _ T _
        _ _ T _ T _ T _


        ye pattern ka use karenge 
    */
    // **** SEE PREVIOUS DP SOLUTION TOO ( pairs ke through aur thoda approach badal ke kaise TLE avoid kiya)
 }
signed main()
{
