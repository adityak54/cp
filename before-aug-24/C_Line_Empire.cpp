...
void Shiremonoga(){
    int n,a,b;
    cin >> n >> a >> b;
    vector<int> v = input<int>(n);
    vector<int> prefSum(n);
    prefSum[0] = v[0];
    fr(i,1,n,1) prefSum[i] = prefSum[i-1]+v[i];
    int ans = prefSum[n-1]*b;
    int prevCapital = 0;
    fr(i,0,n,1){
        int toSub = ((n-i-1)*(v[i]-prevCapital)*b);
        int toMoveCapital = ((v[i]-prevCapital)*a);
        if(toSub>=toMoveCapital){
            // move capital
            prevCapital = v[i];
            ans+=toMoveCapital-toSub;
        }
    }
    // cout << endl;
    cout << ans << endl;
