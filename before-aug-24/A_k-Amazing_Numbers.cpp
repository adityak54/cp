...
void Shiremonoga(){
    ...
    for(auto &it : m){
        int len = it.second[0]+1;
        for(int i=1;i<it.second.size();i++){
            len = max(it.second[i]-it.second[i-1],len);
        }
        len = max(len,n-it.second.back());
        if(ans[len-1]==-1) ans[len-1] = it.first;
        else ans[len-1] = min(ans[len-1],it.first);
    }
    int ind = 0;
    fr(0,n-1,1) if(ans[i]!=-1){ind = i; break;}
    int val = ans[ind];
    fr(ind+1,n-1,1){
