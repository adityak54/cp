...
void Shiremonoga(){
    ...
    for(int i=0;i<n;i++) m[s[i]]++, ind[s[i]].pb(i);
    // for(auto &it:m){
    //     cout<<it.first<<" "<<it.second<<endl;

    // }
    bool chk = true;
    int cnt1 = min(m['N'],m['S']);
    for(int i=0;i<cnt1-1;i++){
        chk?s[ind['N'][i]] = 'R':s[ind['N'][i]] = 'H';
        chk?s[ind['S'][i]] = 'R':s[ind['S'][i]] = 'H';
        chk = !chk;
    }
    if(cnt1>0){
        chk?s[ind['N'][cnt1-1]] = 'R':s[ind['N'][cnt1-1]] = 'H';
        chk?s[ind['S'][cnt1-1]] = 'R':s[ind['S'][cnt1-1]] = 'H';
    }
    if(m.find('N')!=m.end()) m['N']-=cnt1;
    if(m.find('S')!=m.end()) m['S']-=cnt1;


    int cnt2 = min(m['E'],m['W']);
    for(int i=0;i<cnt2-1;i++){
        chk?s[ind['E'][i]] = 'H':s[ind['E'][i]] = 'R';
        chk?s[ind['W'][i]] = 'H':s[ind['W'][i]] = 'R';
        chk = !chk;
    }
    if(cnt2>0){
        chk?s[ind['E'][cnt2-1]] = 'H':s[ind['E'][cnt2-1]] = 'R';
        chk?s[ind['W'][cnt2-1]] = 'H':s[ind['W'][cnt2-1]] = 'R';
    }
    if(m.find('E')!=m.end()) m['E']-=cnt2;
    if(m.find('W')!=m.end()) m['W']-=cnt2;
    for(auto &it:m){
        if(it.second&1){cout<<"NO"<<endl; return;}
    }
    bool check = false;
    for(int i=0;i<n;i++){
        if(s[i]=='N'){
    ...
    if(!chk1 || !chk2){cout<<"NO"<<endl; return;}
    cout<<s<<endl;
