...
class SGTree{
    ...
    void build(int ind,int low,int high,string &s){
        ...
        int cnt = 0;
        fr(i,0,26,1){
            if(sg[2*ind+1][i]==1 || sg[2*ind+2][i]==1){
                cnt++;
                sg[ind][i] = 1;
            }
        }
    }
    void update(int ind, int pos,char val,int low, int high, string &s){
        if(low==high && low==pos){
            fr(i,0,26,1) sg[ind][i]=0; // iss index me jo bhi tha wo hata diye
            sg[ind][val-'a'] = 1;      // naye val waala 1 kar diye
            return;
        }
        int mid = (low+high)>>1;
        if(pos<=mid) update(2*ind+1,pos,val,low,mid,s);
        else update(2*ind+2,pos,val,mid+1,high,s);
    ...
    void query(int ind, int l, int r, int low,int high,vector<int> &check){
        if(low>=l && high<=r){
            fr(i,0,26,1) check[i]= (check[i] || sg[ind][i]);
            return;
        }
        if(low>r || high<l) return;
        int mid = (low+high)>>1;
        query(2*ind+1,l,r,low,mid,check);
        query(2*ind+2,l,r,mid+1,high,check);
...
void Shiremonoga(){
    ...
    while(q--){
        ...
        if(type==1){
            ...
            cin >> ind >> c;
            ind--;
            tree.update(0,ind,c,0,n-1,s);
        }else{
            int l,r;
            cin >> l >> r;
            l--; r--;
            vector<int> check(26);
            tree.query(0,l,r,0,n-1,check);
            int ans = 0;
            fr(i,0,26,1) ans+=check[i];
            cout << ans << endl;
        }
    }
 }
