...
void Shiremonoga(){
    ...
    fr(i,0,n,1){
        if(!col[i]){
            ...
            if(!col[i]) {check2 = false; break;}
        }
    }
    if(!check1 && !check2){cout<<-1<<endl; return;}
    fr(i,0,ans.size(),1){
        cout << ans[i].first << " " << ans[i].second << endl;
    }
    if(check1) fr(i,0,rem1.size(),1) cout << rem1[i].first << " " << rem1[i].second << endl;
    else fr(i,0,rem2.size(),1) cout << rem2[i].first << " " << rem2[i].second << endl;
