...
void Shiremonoga(){
    ...
    // valid -> jisse ham palindrome bana sake (freq of this char >=2)
    int left = 0; int valid = 0;

    for(auto it : m){
        if(it.second&1){
            left++; // odd wale me ek valid nahi hoga baaki hoga
        }
         valid+=it.second/2;
    }
    int ans = (valid/k)*2; 
    left+=(2*valid)-(ans*k); // valid me jo bach gaye 
    if(left>=k) ans++;
    cout<<ans<<endl;
 }
signed main()
