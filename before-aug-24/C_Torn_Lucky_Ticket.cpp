...
void Shiremonoga(){
    ...
    map<string,int> prefSum;
    fr(i,0,n-1,1){
        string s; cin >> s;
        a.pb(s);
        int sum = 0;
        fr(j,0,s.size()-1,1) sum+=a[i][j]-'0';
        prefSum[s] = sum;
        m[s.size()][sum]++;
    }
    int ans = 0;
    fr(i,0,n-1,1){
        ...
        int pichheLen = tsz, aageLen = 0;
        int pichheSum = prefSum[ticket], aageSum = 0;

        fr(j,0,tsz-1,1){
            ans+=m[pichheLen-aageLen][pichheSum-aageSum];
            ans+=m[aageLen-pichheLen][aageSum-pichheSum];
            aageSum+=ticket[j]-'0'; aageLen++;
            pichheSum-=ticket[j]-'0'; pichheLen--;
        }
    }
    cout << ans << endl;
 }
