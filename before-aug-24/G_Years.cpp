...
void Shiremonoga(){
    int n; cin >> n;
    // ans jo hai wo inhi given birth and death dates me se hoga
    // kyuki multiple answers pe minimum waala print karana hai
    map<int,int> alive;
    set<int> s;
    fr(0,n-1,1){
        int birth, death;
        cin >> birth >> death;
//--------- given birth aur death day pe alive kitne hain ----------
        alive[birth]++;
        alive[death]--; 
//------------------------------------------------------------
        s.insert(birth);
