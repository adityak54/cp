...
void Shiremonoga(){
    int n,k; cin >> n >> k;
    vector<pi> occ(k+1,{n+1,-1});
    vector<bool> check(k+1,false);
    for(int i=0;i<n;i++){
        int x; cin >> x;
        check[x] = true;
        occ[x].first = min(i,occ[x].first);
        occ[x].second = max(i,occ[x].second);
    }
    for(int i=k-1;i>0;i--){
        // if(occ[i].first==n+1) continue;
        occ[i].first = min(occ[i+1].first,occ[i].first); 
        occ[i].second = max(occ[i+1].second,occ[i].second); 
    }
    for(int i=1;i<=k;i++){
        if(!check[i]) cout<<0<<" ";
        else cout<<(occ[i].second-occ[i].first+1)*2<<" ";
    }
    cout<<endl;
 }
signed main()
