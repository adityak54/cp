...
void Shiremonoga(){
    ...
    for(auto &it : m){
        ...
        if(it.second.size() & 1) cntOdd++;
    }
    if(cntOdd > 1){cout<<-1<<endl; return;}
    int ans = 0;
    for(auto &it : m){
        vector<int> curr = it.second;
        if(curr.size()&1){
            if(curr.size() == 1) continue;
            vector<int> prefSum(curr.size(),0), suffSum(curr.size(),0);
            for(int i = 1; i < curr.size(); i += 2){
                prefSum[i] = (curr[i] - curr[i-1]) / k;
                if(i > 1) prefSum[i] += prefSum[i-2];
            }
            for(int i = curr.size()-2; i >= 0; i-=2){
                suffSum[i] = (curr[i+1] - curr[i]) / k;
                if(i < curr.size()-2) suffSum[i] += suffSum[i+2];
            }
            // print(prefSum);
            // print(suffSum);
            int currAns = INT_MAX;
            ...
            for(int i = 0; i < curr.size(); i+=2){
                int mn = 0;
                if(i > 0) mn += prefSum[i-1];
                if(i < curr.size()-1) mn += suffSum[i+1];
                currAns = min(currAns, mn);
            }
            ans += currAns;      
        }else{
            int mn = 0;
            for(int i = 0; i < curr.size(); i+=2){
                mn += (curr[i+1] - curr[i])/k;
            }
            ans += mn;
        }
    }
