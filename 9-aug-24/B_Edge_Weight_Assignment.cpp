...
//-------------------------CODE----------------------
bool dfs(int node, int parent, int currDist, vector<int> adj[]){
    if(adj[node].size() == 1 && currDist) return true;
    for(auto &it : adj[node]){
        if(it == parent) continue;
        if(dfs(it, node, !currDist, adj)) return true;
    }
    return false;
}
void Shiremonoga(){
    ...
    map<int,int> leafNeighbors;
    int leaf = -1;
    for(int i = 1; i <= n; i++){
        if(adj[i].size() == 1){
            leaf = i;
            leafNeighbors[adj[i][0]]++;
        }
    }
    bool oddPath = dfs(leaf, 0, 0, adj);
    int ans = n-1; // assumed all the weights are unique
    for(auto it : leafNeighbors){
        ans -= it.second - 1; // last edge pe jo path ka total xor hai wahi daalenge, to make it 0
                              // therefore unique wrights it.second-1 number se kam ho jayenge
    }
    cout << (oddPath ? 3 : 1) << " " << ans << endl;
 }
signed main()
{
