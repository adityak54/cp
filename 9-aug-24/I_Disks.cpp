...
//-------------------------CODE----------------------
int cnt[2], vis[1001], color[1001];
bool flag = 0;
void dfs(int node, int currColor, vector<int> adj[]){
    vis[node] = 1;
    color[node] = currColor;
    cnt[currColor]++;
    for(auto &it: adj[node]){
        if(!vis[it]){
            dfs(it, !currColor, adj);
...
void Shiremonoga(){
    ...
    // formed graph of tangent circles
    vector<int> adj[n];
    for(int i = 0; i < n; i++){
        for(int j = i+1; j < n; j++){
            int x1 = c[i][0], y1 = c[i][1], x2 = c[j][0], y2 = c[j][1];
            if((y2-y1)*(y2-y1) + (x2-x1)*(x2-x1) == (c[i][2] + c[j][2])*(c[i][2] + c[j][2])){
                adj[i].push_back(j);
                adj[j].push_back(i);
            }
        }
    ...
    for(int i = 0; i < n; i++){
        if(!vis[i]){
            ...
            flag = 0;
            dfs(i, 0, adj);
            // bipartite + odd nodes in the component
            if(cnt[0] != cnt[1] && !flag){cout<<"YES"<<endl; return;}
        }
    }
    cout << "NO" << endl;
 }
