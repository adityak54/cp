...
void Shiremonoga(){
    ...
    vector<vector<int>> mx(n,vector<int>(m,INT_MIN)), mn(n,vector<int>(m,INT_MAX));
    mx[0][0] = mn[0][0] = grid[0][0];
    for(int i = 1; i < n; i++){
        mx[i][0] = grid[i][0] + mx[i-1][0];
        mn[i][0] = grid[i][0] + mn[i-1][0];
    }
    for(int i = 1; i < m; i++){
        mx[0][i] = grid[0][i] + mx[0][i-1];
        mn[0][i] = grid[0][i] + mn[0][i-1];
    }
    for(int i = 1; i < n; i++){
        for(int j = 1; j < m; j++){
    ...
    // cout << endl;
    // for(int i = 0; i < n; i++){
    //     for(int j = 0; j < m; j++) cout << mn[i][j] << " ";
    //     cout << endl;
    // }
    // cout << mx[n-1][m-1] << " " << mn[n-1][m-1] << endl;
