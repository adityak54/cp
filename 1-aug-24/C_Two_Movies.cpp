...
void Shiremonoga(){
    int n; 
    cin >> n;
    vector<int> a=input<int>(n), b = input<int>(n);
    int movie1 = 0, movie2 = 0, bothNeg = 0, bothPos = 0;
    for(int i = 0; i < n; i++){
        if(a[i] == 1 && b[i] == 1) bothPos++;
        else if(a[i] == -1 && b[i] == -1) bothNeg++;

        else if(a[i] == 1) movie1++;
        else if(b[i] == 1) movie2++;
    }
    int diff = abs(movie1-movie2);
    if(bothPos){
        if(movie1 < movie2){
            movie1 += min(diff, bothPos);
            bothPos -= min(diff, bothPos);
        }
        else if(movie2 < movie1){
            movie2 += min(diff, bothPos);
            bothPos -= min(diff, bothPos);
        }
    ...
    diff = abs(movie1-movie2);
    if(bothNeg){
        if(movie1 < movie2){
            movie2 -= min(diff, bothNeg);
            bothNeg -= min(diff, bothNeg);
        }
        else if(movie2 < movie1){
            movie1 -= min(diff, bothNeg);
            bothNeg -= min(diff, bothNeg);
        }
