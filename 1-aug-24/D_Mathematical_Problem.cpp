...
int f(int i, int num, bool put, int cannotPut){
    if(i == n){
        return num;
    }
    if(put && i-1!=cannotPut){
        int val = f(i, 0, false, cannotPut);
        return min(num + val, num * val);
    }
    num = num*10 + (s[i]-'0');
    return f(i+1, num, true, cannotPut);
}
void Shiremonoga(){
    cin >> n;
    cin >> s;
