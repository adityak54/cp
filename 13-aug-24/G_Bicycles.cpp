...
void Shiremonoga(){
    ...
    vector<int> s=input<int>(n);
    priority_queue<ppi,vector<ppi>,greater<ppi>> pq;
    vector<int> dist(n,LONG_LONG_MAX), bikes(n), mnDist(n,LONG_LONG_MAX);
    for(int i = 0; i < n; i++) bikes[i] = s[i];
    dist[0] = mnDist[0] = 0;
    pq.push({0,{s[0], 0}});
    while(!pq.empty()){
        int currWeight = pq.top().first;
        int node = pq.top().second.second;
        int currBike = pq.top().second.first;
        // cout << currWeight << " " << currBike << " " <<  node<< endl;
        pq.pop();
        
        for(auto &it : adj[node]){
            int weight = it.second, nextNode = it.first;
            int bike = min(s[node], currBike);
            if(dist[nextNode] > currWeight+(weight*bike)){
                dist[nextNode] = currWeight+(weight*bike);
                mnDist[nextNode] = min(mnDist[nextNode], dist[nextNode]);
                pq.push({dist[nextNode],{bike, nextNode}});
                bikes[nextNode] = bike;
            }else if(bikes[nextNode] > bike){
                dist[nextNode] = currWeight+(weight*bike);
                mnDist[nextNode] = min(mnDist[nextNode], dist[nextNode]);
                pq.push({dist[nextNode],{bike, nextNode}});
                bikes[nextNode] = bike;
