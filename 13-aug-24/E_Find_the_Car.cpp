...
void Shiremonoga(){
    cin >> n >> k >> q;
    vector<int> a = input<int>(k);
    vector<int> b = input<int>(k);
    for(int i = 0; i < q; i++){
        int pos; 
        cin >> pos;
        int ind = lower_bound(a.begin(), a.end(), pos)-a.begin();
        if(a[ind] == pos) cout << b[ind] <<  " ";
        else{
            int distance =  pos;
            if(ind > 0) distance -= a[ind-1];
            int totalDistance = a[ind], totalTime = b[ind];
            if(ind > 0) totalDistance -= a[ind-1], totalTime -= b[ind-1];
            // int speed = totalDistance / totalTime;
            int ans = (distance * totalTime) / totalDistance;
            if(ind > 0) ans += b[ind-1];
            cout << ans << " ";
        }
    }
    cout << endl;
