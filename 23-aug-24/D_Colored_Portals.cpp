...
vector<string> possible = {"BR", "BG", "BY", "GR", "GY", "RY"};
void Shiremonoga(){
    int n,q;
    cin >> n >> q;
    ...
    for(int i = 0; i < q; i++){
        ...
        cin >> l >> r;
        l--, r--;
        string col1 = v[l], col2 = v[r];
        if(match(col1,col2)) cout << abs(r-l) << endl;
        else{
            int mn = INT_MAX;
            for(int p = 0; p < 6; p++){
                if(possible[p] == col1 || possible[p] == col2) continue;
                auto it  = m[possible[p]].lower_bound(max(l,r));
                if(it != m[possible[p]].end()){
                    mn = min(mn, abs(*it-l) + abs(*it-r));
                }
                if(it != m[possible[p]].begin()){
                    it--;
                    mn = min(mn, abs(*it-l) + abs(*it-r));
                }
